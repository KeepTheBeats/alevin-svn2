package vnreal.generators.topologies.transitstub;
...
import vnreal.generators.topologies.transitstub.generators.AGenerator;
import vnreal.generators.topologies.transitstub.generators.WaxmanGenerator;
...
import vnreal.generators.topologies.transitstub.util.MeanRNG;
...
public class TransitStub {
	...
	public static Graph generate(
		...
		final Graph domainGraph =
				domainGenerator.generateConnected(numDomains, rnd);
		...
		for (Entry<Vertex, Graph> e : transitGraphs.entrySet()) {
			...
			for (Vertex transitVert : transitGraph.getVertices()) {
				...
				MeanRNG stubSize = new MeanRNG(
						numStubs, numMeanStubVerts, RANDMULT, rnd);
