...
public class ConstraintsGeneratorDialog extends AbstractButtonDialog {
	...
	public ConstraintsGeneratorDialog() {
	...
	protected JPanel createContent() {
		...
		for (Class<?> res : getAllResources()) {
			...
			/*TODO: There seems to be a problem with resources that don't have setters defined.
			 * At any rate, the current solution here (using introspection to find setters) is
			 * pretty ugly. There should be a better solution to this! */
//			if (resParamNameList.isEmpty()) {System.out.println("Class name: " + res.getSimpleName()); }
		...
		for (Network<?, ?, ?> layer : scenario.getNetworkStack().getVirtuals()) {
			...
			demandsTabbedPane.addTab("VN " + layer.getLayer(), tablePanel);
		}
	...
	private Class<?>[] getAllDemands() {
		Class<?>[] nodeDem = ClassScanner.getClassesImplementing(
				this.getClass(), "vnreal.constraints.demands", INodeConstraint.class);
		Class<?>[] linkDem = ClassScanner.getClassesImplementing(
				this.getClass(), "vnreal.constraints.demands", ILinkConstraint.class);
	...
	private Class<?>[] getAllResources() {
		Class<?>[] nodeRes = ClassScanner.getClassesImplementing(
				this.getClass(), "vnreal.constraints.resources", INodeConstraint.class);
		Class<?>[] linkRes = ClassScanner.getClassesImplementing(
				this.getClass(), "vnreal.constraints.resources", ILinkConstraint.class);
