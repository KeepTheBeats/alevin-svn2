...
public class OptimalEnergyAlgorithmHiddenHop extends GenericMappingAlgorithm {
	...
	public OptimalEnergyAlgorithmHiddenHop(AlgorithmParameter param) {
		
		boolean nodeOverload = param.getBoolean("overload", DEFAULT_OVERLOAD);
		int distance = param.getInteger("distance", DEFAULT_DIST);
		
		this.nodeMappingAlgorithm = new OptimalEnergyMappingHiddenHop(distance, nodeOverload);
		/*
		this.nodeMappingAlgorithm.setMaxSubsNodeGrade(MiscelFunctions.maxNodeGrade(stack
				.getSubstrate()));
