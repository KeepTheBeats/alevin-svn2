...
/**
 * This is the random WaxMan Generator
 * 
 * @author Fabian Kokot
 *
 */
@GeneratorParameter(
		parameters = { "TR:Waxman_alpha", "TR:Waxman_beta" , "TR:SNetSize", "TR:NumVNodesPerNet", "TR:NumVNets"}
) 
public class WaxmanNetworkStackGenerator extends AbstractNetworkStackGenerator {
	...
	public NetworkStack generate(ArrayList<Object> parameters) {
		...
		SubstrateNetwork sn = createSubstrateNetwork(alpha, beta, sNetSize);
		
		List<VirtualNetwork> vns = createVirtualNetworks(alpha, beta, numVNets, numVNodesPerNet);
