...
/**
 * This metrics calculates the average Security-Spread between all virtual Nodes
 * co-hosted on one Substrate-Node after the mapping.
 * 
 * @author Julian Willerding
 */
public class AvSecSpreadDemProv implements EvaluationMetric<NetworkStack> {
	...
	public double calculate(NetworkStack stack) {
		...
		// Visit all Substrate-Nodes
        	for (SubstrateNode sn : sNetwork.getVertices()) {
        	    double returnedValue = VnrUtils.getSecuritySpreadDemProv(sn);
        	    
        	    if (returnedValue != -1) {
        		++visitedNodes;
        		maxOccuredSecuritySpread += returnedValue;
        	    }
        	}

		return (maxOccuredSecuritySpread / visitedNodes);
	}

	public String toString() {
		return "AvSecSpreadDemProv";
	}
