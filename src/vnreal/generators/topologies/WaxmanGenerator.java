...
/**
 * This class implements an {@link ITopologyGenerator} that uses the
 * {@link WaxmanGraphGenerator} to generate a topology.
 * 
 * @author Philip Huppert
 */
public class WaxmanGenerator implements ITopologyGenerator {
	...
	public SubstrateNetwork generateSubstrateNetwork(
		...
		}.generateGraph(this.numNodes, this.alpha, this.beta, random);
	}
		...
		valSeed.getDocument().addDocumentListener(new DocumentListener() {
			...
			private void update() {
				try {
					...
					seed = (long) valSeed.getText().hashCode();
				}
			}
		});
	...
	private abstract class Generator<T extends Network, N extends Node, L extends Link> {
		...
		public T generateGraph(int numNodes, double alpha, double beta, Random random) {
			...
			WaxmanGraphGenerator<N, L> sgg =
					new WaxmanGraphGenerator<N, L>(random, alpha, beta, false);
