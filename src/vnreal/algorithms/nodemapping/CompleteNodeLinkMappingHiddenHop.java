...
public class CompleteNodeLinkMappingHiddenHop extends AbstractNodeMapping {
	...
	public CompleteNodeLinkMappingHiddenHop(
			int dist, double wCpu, double wBw, boolean nodeOverload) {
		super(nodeOverload);
		this.distance = dist;
		this.wCpu = wCpu;
		this.wBw = wBw;
	...
	protected boolean nodeMapping(SubstrateNetwork sNet, VirtualNetwork vNet) {
		candiNodes = createCandidateSet(sNet, vNet);
	...
	private boolean performLinkMapping(SubstrateNetwork sNet, VirtualNetwork vNet,
		...
		for (Iterator<VirtualLink> links = vNet.getEdges().iterator(); links
			...
			if (!corrSrcSNode.getName().equals(corrDstSNode.getName())) {
				for (Iterator<List<String>> cad = flow.keySet().iterator(); cad
					...
					if (flowPercentages.get(flowValues) == null) {
						...
						tmpResult = MiscelFunctions
								.round(flowPercentages.get(flowValues), 2);
					}
					if (tmpResult != 0) {
						if (corrSrcSNode.getId() == Integer.parseInt(flowValues
							...
							newBwDem.setDemandedBandwidth(MiscelFunctions
									.round(VlOriginalDemand	* flowPercentages.get(flowValues), 3));
	 ...
	 */
	private SubstrateNode getSnodeById(SubstrateNetwork sNet, int id) {
