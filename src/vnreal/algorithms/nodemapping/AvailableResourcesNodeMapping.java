...
public class AvailableResourcesNodeMapping extends AbstractNodeMapping {
	 ...
	 *            substrate Network
	 * @param distance
	 *            Indicates the maximum distance within which to search for
	 *            candidate substrate nodes. If set to a negative value, distance
	 *            is ignored.
	 * @param nodesOverload
	 *            Indicates if the fact that more that one node in one VNR are
	 *            mapped to the same node in the SN
	 */
	@Deprecated
	public AvailableResourcesNodeMapping(int distance,
	...
	public AvailableResourcesNodeMapping(int distance, boolean nodesOverload) {
		super(nodesOverload);
		this.distance = distance;
	}
	 ...
	 *         resources value
	 */
	private SubstrateNode bestAvailableResources(SubstrateNetwork sNet, List<SubstrateNode> candidates) {
		double resCPU = 0;
		double resLink = 0;
		double greatAr = 0;
