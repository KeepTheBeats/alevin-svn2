...
public class BFSNRnodeMappingPathSplitting extends AbstractNodeMapping {
	...
	public BFSNRnodeMappingPathSplitting(
			double wCpu, double wBandwidth, boolean subsNodeOverload, double epsilon,
			int dist, int maxHop) {
		super(subsNodeOverload);
		this.epsilon = epsilon;
		this.dist = dist;
	...
	protected boolean nodeMapping(SubstrateNetwork sNet, VirtualNetwork vNet) {
		...
		for (int i = 0; i < bfsQueue.size(); i++) {
			...
			if (currCandidateSet.get(currVnode).isEmpty()
					|| !match(sNet, vNet, currVnode, currCandidateSet.get(currVnode))) {
				backtrack_count++;
				i = i - 2;
				wasBacktrack = true;
			} else {
	...
	private boolean MapPathsWithMappedNodes(SubstrateNetwork sNet, VirtualNetwork vNet,
		...
		for (VirtualLink vLink : vNet.getOutEdges(vNode)) {
			...
			if (nodeMapping.containsKey(vNet.getDest(vLink))
				...
				if (!performLinkMapping(sNet, sNode, dstNode, vLink, vNet)) {
					return false;
				}
			}
		}
		for (VirtualLink vLink : vNet.getInEdges(vNode)) {
			...
			if (nodeMapping.containsKey(vNet.getSource(vLink))
				...
				if (!performLinkMapping(sNet, srcNode, sNode, vLink, vNet)) {
					return false;
				}
			}
		}
	...
	private boolean performLinkMapping(SubstrateNetwork sNet, SubstrateNode srcSnode,
		...
		if (problemSolver.problemFeasible()) {
			...
			for (Iterator<List<String>> cad = solverResult.keySet().iterator(); cad
				...
				Double vtmp = MiscelFunctions.round(solverResult.get(tmpValues), 12);
