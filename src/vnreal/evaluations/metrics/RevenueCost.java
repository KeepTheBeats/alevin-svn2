...
public class RevenueCost implements EvaluationMetric<NetworkStack> {
	...
	public RevenueCost(boolean isPsAlgorithm) {
	...
	public double calculate(NetworkStack stack) {
		...
		double c = cost.calculate(stack);
		if (Double.isNaN(c))
			return Double.NaN;
		
		double RevenueCost = rev.calculate(stack) / c;
		return (RevenueCost);
	}
