...
public class MlsNodeMapping_BestFit_Lower extends AbstractNodeMapping {
	...
	public MlsNodeMapping_BestFit_Lower(SubstrateNetwork sNet, boolean subsNodeOverload, int alpha) {
	...
	@Override
	public boolean nodeMapping(SubstrateNetwork sNet, VirtualNetwork vNet) {		
	 ...
	 */
	private boolean preMapping(SubstrateNetwork sNet){
	 ...
	 */
	private boolean mapNetwork(SubstrateNetwork sNet) {
		this.phase = "Mapping";
		snodeComperator sComp = new snodeComperator();
	 ...
	 * @return true if ok, otherwise false
	 */
	private boolean map_flex(SubstrateNetwork sNet, VirtualNode vn) {
		ArrayList<SubstrateNode> tmpCandidates = findCandidates(sNet, vn, true);
		if(!tmpCandidates.isEmpty()) {
