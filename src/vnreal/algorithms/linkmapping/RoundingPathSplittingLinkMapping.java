...
public class RoundingPathSplittingLinkMapping extends AbstractLinkMapping {
	 ...
	 * Method constructor
	 * 
	 * @deprecated Rounding type is now assigned with a boolean (as should be) 
	 * @param cpuWeight
	 ...
	 */
	@Deprecated
	public RoundingPathSplittingLinkMapping(
	...
	public RoundingPathSplittingLinkMapping(
			double cpuWeight, double bwWeight, int k, boolean randomize) {
		this.wBw = bwWeight;
		this.wCpu = cpuWeight;
		this.k = k;
		...
		if (problemSolver.problemFeasible()) {
			...
			for (Iterator<VirtualLink> links = vNet.getEdges().iterator(); links
				...
				if (!srcSnode.equals(dstSnode)) {
					...
					if (randomize) {
						 ...
						 */
						mappedPath = randomizedElection(sNet, srcSnode,
								candidatePaths, tmpl);
					} else {
						/*
						 ...
						 */
						mappedPath = greatest(sNet, srcSnode, candidatePaths, tmpl);
					}
			...
			if (!unMappedLinks.isEmpty()) {
				for (VirtualLink unMappedVl : unMappedLinks) {
					...
					Yen<SubstrateNode, SubstrateLink> kshortestPaths = new Yen<SubstrateNode, SubstrateLink>(
							sNet, linkWeight);
	 ...
	 */
	private List<SubstrateLink> randomizedElection(SubstrateNetwork sNet, SubstrateNode srcSNode,
