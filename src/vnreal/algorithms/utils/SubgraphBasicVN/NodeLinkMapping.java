...
public class NodeLinkMapping {
	...
	private Collection<ResourceDemandEntry> mappings;

	// TODO: implement hashCode in NetworkEntity class
	public NodeLinkMapping() {
		clear();
	}

	public NodeLinkMapping(NodeLinkMapping copy) {
		...
		mappings = new LinkedList<ResourceDemandEntry>(copy.mappings);
	}
	...
	public void clear() {
		...
		mappings = new LinkedList<ResourceDemandEntry>();
	}

	public Collection<ResourceDemandEntry> occupyAllResources(
			SubstrateNetwork sNetwork) {
		for (Entry<VirtualNode, SubstrateNode> e : nodes.entrySet()) {
			mappings.addAll(Utils.occupyResources(e.getKey().get(), e.getValue().get()));
		}
		for (Entry<VirtualLink, List<SubstrateLink>> e : links.entrySet()) {
			mappings.addAll(Utils.occupyPathResources(e.getKey(), e.getValue(),
					sNetwork));
		}
		return new LinkedList<ResourceDemandEntry>(
				mappings);
	}
	...
	public void remove(VirtualLink vl, List<SubstrateLink> sl) {
		Collection<VirtualLink> list = linkMappings.get(sl);
		list.remove(vl);
		links.remove(vl);
		mappedVLinks--;
