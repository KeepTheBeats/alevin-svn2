...
import vnreal.generators.topologies.AlFaresGenerator;
import vnreal.generators.topologies.ExponentialGenerator;
import vnreal.generators.topologies.FatTreeGenerator;
import vnreal.generators.topologies.GridGenerator;
import vnreal.generators.topologies.ITopologyGenerator;
import vnreal.generators.topologies.LocalityGenerator;
import vnreal.generators.topologies.RandomEdgeGenerator;
import vnreal.generators.topologies.RingGenerator;
import vnreal.generators.topologies.StarGenerator;
import vnreal.generators.topologies.TransitStubGenerator;
 ...
 * {@link ITopologyGenerator} for each network.
 * 
...
public class MultiAlgoScenarioWizard extends AbstractButtonDialog {
	...
	protected void doAction() {
		...
		ITopologyGenerator[] generators = this.model.getGenerators();
	 ...
	 * {@link ITopologyGenerator}s for the different networks.
	 * 
	 * @author Philip Huppert
	 */
	private class WizardModel extends AbstractTableModel {
		...
		private List<ITopologyGenerator> generators;
	
		public WizardModel() {
			super();
			this.generators = new LinkedList<ITopologyGenerator>();
		...
		public void addGenerator(int i) {
			if (this.canAddGenerator(i)) {
				...
				ITopologyGenerator gen = new WaxmanGenerator();
		...
		/**
		 * @return the list of {@link ITopologyGenerator}s configured by the
		 *         user.
		 */
		public ITopologyGenerator[] getGenerators() {
			return this.generators.toArray(new ITopologyGenerator[0]);
		}

		public void setGenerator(int row, ITopologyGenerator generator) {
			this.generators.set(row, generator);
			this.fireTableRowsUpdated(row, row);
		}

		public ITopologyGenerator getGenerator(int row) {
	...
	private class ContextHandler implements ActionListener {
		private ITopologyGenerator clipboard = null;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (cmd.equals(CHANGE_GENERATOR_ACTN)) {
				ITopologyGenerator generator = model.getGenerator(row);
			...
			// Paste the configuration.
				...
				ITopologyGenerator current = model.getGenerator(row);
	 ...
	 * change the {@link ITopologyGenerator} associated with a network in the
	 * table.
	 * 
	 * @author Philip Huppert
	 */
	private class GeneratorChange extends AbstractButtonDialog {
		...
		private ITopologyGenerator generator;
		private Map<String, String> classnames;
		private JComboBox<String> dropdown;
		private JPanel panel;
	
		public GeneratorChange(ITopologyGenerator current) {
			super(
					GUI.getInstance(),
					CHGDIAG_TITLE,
					CHGDIAG_ACCEPT,
			...
			for (String classname : GENERATORS) {
				ITopologyGenerator generator = this.constructGenerator(classname);
				if (generator != null) {
		...
		/**
		 * @return the {@link ITopologyGenerator} selected by the user.
		 */
		public ITopologyGenerator getGenerator() {
			return this.generator;
		}

		private ITopologyGenerator constructGenerator(String classname) {
			try {
				Class<?> clazz = Class.forName(classname);
				Constructor<?> constr = clazz.getConstructor();
				Object inst = constr.newInstance(new Object[0]);
				return (ITopologyGenerator) inst;
