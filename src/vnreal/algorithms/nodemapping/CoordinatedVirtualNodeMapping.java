...
public class CoordinatedVirtualNodeMapping extends AbstractNodeMapping {
	 ...
	 * 
	 * @deprecated sNet is ignored now.
	 * @param sNet
	 * @param dist Distance to select the candidate nodes to be mapped.
	 * @param wCpu
	 * @param wBw
	 * @param type Indicates the rounding type (deterministic, if 0, randomized otherwise) 
	 * @param nodeOverload
	 */
	@Deprecated
	 ...
	 * 
	 * @param dist Distance to select the candidate nodes to be mapped.
	 * @param wCpu CPU weight
	 * @param wBw Bandwidth weight
	 * @param randomize Indicates the rounding type (deterministic, if false, randomized otherwise) 
	 * @param nodeOverload
	 */
	public CoordinatedVirtualNodeMapping(int dist,
			double wCpu, double wBw, boolean randomize, boolean nodeOverload) {
		super(nodeOverload);
		this.distance = dist;
	...
	protected boolean nodeMapping(SubstrateNetwork sNet, VirtualNetwork vNet) {
		...
		if (augmentedCreation(sNet, vNet, unMappedVirtualNodes,
			...
			return virtualNodeMapping(sNet, unMappedVirtualNodes, augmentedSubstrate,
					augmentedCorrespond, solverResult, solverResultFlow);
		} else {
			return false;
		}
	}
	 ...
	 * @return
	 */
	private boolean augmentedCreation(SubstrateNetwork sNet, VirtualNetwork vNet,
		...
		double maxNodeResource = maxNodeResource(sNet);
	...
	private boolean virtualNodeMapping(SubstrateNetwork sNet, List<VirtualNode> unmappedvNodes,
		...
		for (Iterator<VirtualNode> vn = unmappedvNodes.iterator(); vn.hasNext();) {
			...
			for (Iterator<List<String>> cad = resultFlow.keySet().iterator(); cad
				...
				if ((mappedNode.getId() == Long.parseLong(tmpValues.get(2)) || mappedNode
						.getId() == Long.parseLong(tmpValues.get(3)))
						&& vtmp != 0) {

					if (mappedNode.getId() == Long
							.parseLong(tmpValues.get(2))) {
						tmpSNode = getSubstrateNode(sNet, Long.parseLong(tmpValues
								.get(3)));
			...
			for (Iterator<List<String>> cad = result.keySet().iterator(); cad
				...
				if ((mappedNode.getId() == Long.parseLong(tmpValues.get(0)))
						&& vtmp != 0) {
					tmpSNode = getSubstrateNode(sNet, Long.parseLong(tmpValues
							.get(1)));
					if (candiNodesWeight.get(tmpSNode) != null)
